...
export async function saveActiveSession(session:ShoppingSession){
...
export async function createList(name:string,session:ShoppingSession){
  const lists = await ensureLists();
  const now = new Date().toISOString();
...
export async function selectList(id:string){
  const lists = await ensureLists();
