...
export default function ScannerScreen(){
  ...
  const [reviewQuantity,setReviewQuantity] =
    useState("1");

  const [saveMessage,setSaveMessage] =
    useState("");

  const [duplicateReadyToAdd,setDuplicateReadyToAdd] =
    useState(false);
  ...
  async function addReviewedItem(){
    ...
    try{
      ...
      if(duplicateFound && duplicateItemId){
        ...
        setSavingItem(false);
        resetScanner();
        showScanToast("Quantity updated");
        return;
    ...
    if(isShoppingListMode){
      ...
      try{
        ...
        setReviewName(
          duplicateItem
            ? String(
                (duplicateItem as {name?:unknown}).name
                ||
                finalName
              )
            : finalName
        ...
        setReviewPrice(
          ...
          &&
          Number(
            (duplicateItem as {price?:unknown}).price
            ||
            0
          ) > 0
            ? String(
            ...
            !scanned
              ? (
                <Text style={styles.hint}>
                  Point the front camera at the barcode and hold steady
                </Text>
              )
              : (
                <Text style={styles.hint}>
                  Move the camera over the barcode
                </Text>
              )
